...
export default function RecipesScreen() {
  ...
  const loadCookbooks = useCallback(async (isActive: () => boolean = () => true) => {
    ...
    try {
      ...
      setCookbooksError(err instanceof Error ? err.message : "Could not load cookbooks.");
  ...
  useEffect(() => {
    ...
    if (segment !== "cookbooks") return;

    void loadCookbooks();

  }, [segment, loadCookbooks]);
  ...
  async function onRefresh() {
    ...
    try {
      ...
      await Promise.all([loadCookbooks(), runSearch(query)]);
        ...
        <>
          ...
          {cookbooksError ? (
            ...
            <AppText variant="body" color="danger">
              ...
              {cookbooksError} Pull down to retry.

            </AppText>

          ) : null}

          {cookbooksLoading && cookbooks.length === 0 && !cookbooksError ? (
              ...
              <AppText variant="body" color="muted">
                ...
                {query.trim()
                  ...
                  : cookbooksError
                    ...
                    ? "Your cookbooks could not be loaded."

                    : "No cookbooks yet. Create one to group recipes."}

              </AppText>

              {!query.trim() && !cookbooksError ? (
